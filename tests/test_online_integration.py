...
class OnlineIntegrationTests(unittest.TestCase):
    ...
    def test_exact_ocr_returns_scene_and_best_keyframe(self) -> None:
        result = run(self.service().search(SearchRequest(query='"Gừng cay muối mặn"', task=TaskType.TEXTUAL_KIS, top_k=1)))
        ...
        # frame_idx là tọa độ nộp bài — phải khớp keyframe được chọn.
        self.assertEqual(hit.best_frame_idx, 750)
    ...
    def test_sequence_is_same_video_and_increasing(self) -> None:
        ...
        self.assertEqual(result.sequences[0].frame_ids, [150, 450, 750])
