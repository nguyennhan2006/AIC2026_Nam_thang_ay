...
class SearchService:
    ...
    async def _hydrate(self, candidates: list[Candidate]) -> list[SearchHit]:
        documents = {
            item.scene_id: item
    ...
    async def search(self, request: SearchRequest) -> SearchResponse:
        ...
        if request.task == TaskType.SEQUENCE and len(plan.events) >= 2:
            ...
            for event in plan.events:
                ...
                event_hit_lists.append(await self._hydrate(candidates))
        ...
        hits = await self._hydrate(candidates[: request.top_k])
