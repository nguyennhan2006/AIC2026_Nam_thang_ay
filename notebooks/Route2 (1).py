"""
AIC 2026 - Caption/Detect frame bang model Qwen3-VL tu host tren server rieng (vLLM).
...
CACH DUNG:
    python caption_frames_selfhosted.py
...
if USE_OPENROUTER:
    # Goi qua OpenRouter (giong notebook) - lay key tu file .env canh script (khong hard-code).
...
# pipeline khac (TransNetV2 + CLIP keyframe selection) - moi scene la 1 subfolder
# "{video_id}_S{scene_idx}" (vd K16_V001_S0001), giu nguyen cau truc do.
FRAMES_DIR = Path("./real_input/01_keyframes_output/keyframes")
...
KEYFRAMES_METADATA_JSON = Path("./real_input/01_keyframes_output/keyframes.json")

# Output (JSONL - 1 JSON object/dong, append duoc truc tiep khong can doc lai ca file)
OUT_DIR = Path("./aic_openrouter_captions")
...
def _load_keyframes_metadata() -> Dict[str, Dict[str, Any]]:
    ...
    if KEYFRAMES_METADATA_JSON.exists():
        try:
            with KEYFRAMES_METADATA_JSON.open("r", encoding="utf-8") as f:
                data = json.load(f)
            for item in data:
